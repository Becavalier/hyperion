...
function spawnSparks(x: number, y: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const dy = Math.sin(angle) * dist - 4; // slight upward bias
    el.style.setProperty("--sx", `${x}px`);
    el.style.setProperty("--sy", `${y}px`);
    el.style.setProperty("--dx", `${dx}px`);
    el.style.setProperty("--dy", `${dy}px`);
    el.style.animationDuration = `${400 + Math.random() * 200}ms`;
  ...
  async function handleRate(rating: SelfRating, event?: { currentTarget: HTMLElement }) {
    ...
    try {
      ...
      try {
        ...
        const prevScoreRaw = localStorage.getItem("hacker.score");
        ...
        localStorage.setItem("hacker.score", String(newScore));
      } catch {
      ...
      {q.category === "quiz" ? (
        ...
        >
          ...
          <div className="flex flex-col flex-1 overflow-hidden min-h-0">
            ...
            {/* ── DONE: already rated ── */}
            {review && qs.reviewMode !== "manual" ? (
              <div className={cn("flex-1 flex flex-col items-center justify-center gap-3", ratingColor[review.self_rating])}>
                <CheckCircle2 className="w-8 h-8" style={{
                  color: review.self_rating === "mastered" ? "#00ff41" : review.self_rating === "fuzzy" ? "#ffb300" : "#ff3358"
                }} />
                ...
                <div className="flex gap-2 mt-1">
                  {qs.code.trim() && (
                    <button onClick={() => patch(q.id, { reviewMode: "manual" })}
                      className="text-xs border border-[#1e321e] text-[#4d7a4d] px-3 py-1.5 hover:border-[#2a402a] hover:text-[#b8f5b8] tracking-wider transition-colors">
                      VIEW_CODE
                    </button>
                  )}
                  <button
                    onClick={handleReset}
                    disabled={resetting}
                    className="flex items-center gap-1.5 text-xs border border-[#1e321e] text-[#2a402a] px-3 py-1.5 hover:border-[#ff3358] hover:text-[#ff3358] hover:bg-[#120004] disabled:opacity-30 tracking-wider transition-colors"
                  >
                    <RotateCcw className="w-3 h-3" />
                    {resetting ? "···" : "RESET"}
                  </button>
              ...
              <>
                ...
                <div className="px-4 py-2.5 border-t border-[#1e321e] bg-[#080c08] flex items-center justify-between shrink-0">
                  ...
                  <button onClick={() => patch(q.id, { phase: "submitted" })}
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                  ...
                  {!qs.reviewMode && (
                    <div className="flex-1 flex flex-col items-center justify-center gap-5">
                      <p className="text-[#2a402a] text-xs tracking-widest">// SELECT_REVIEW_MODE</p>
                      <div className="flex gap-3">
                        <button onClick={handleAIReview}
                          className="flex items-center gap-2 px-5 py-2 text-xs border border-[#00d4ff] text-[#00d4ff] hover:bg-[#00111a] tracking-wider transition-colors">
                          <Sparkles className="w-3.5 h-3.5" /> AI_REVIEW
                        </button>
                        <button onClick={() => patch(q.id, { reviewMode: "manual" })}
                          className="flex items-center gap-2 px-5 py-2 text-xs border border-[#b8f5b8] text-[#b8f5b8] hover:bg-[#001a00] tracking-wider transition-colors">
                          MANUAL_REVIEW
                        </button>
                      </div>
                      <p className="text-[#1e321e] text-xs">— or rate directly below —</p>
                    </div>
                  )}
                  ...
                  {qs.reviewMode === "ai" && (
                    <>
                      <div className="px-4 py-2 border-b border-[#1e321e] bg-[#080c08] text-[#00d4ff] text-xs tracking-widest flex items-center gap-2 shrink-0">
                        <Sparkles className="w-3 h-3" /> // AI_FEEDBACK
                      </div>
                  ...
                  {/* Manual: side-by-side code comparison */}
                  {qs.reviewMode === "manual" && (
                    <div className="flex-1 flex flex-col sm:grid sm:grid-cols-2 overflow-hidden">
                      <div className="border-b sm:border-b-0 sm:border-r border-[#1e321e] overflow-hidden flex flex-col min-h-[180px] sm:min-h-0">
                        <CodeEditor value={qs.code || "// (empty)"} readOnly label="YOUR_ANSWER" />
                      </div>
                      <div className="overflow-hidden flex flex-col min-h-[180px] sm:min-h-0">
                        <CodeEditor value={q.answer_hint ?? "// no reference available"} readOnly label="REFERENCE" />
                      </div>
                    </div>
                  )}
                </div>

                {/* Rating bar — always shown after submit */}
                <div className="px-4 py-2.5 border-t border-[#1e321e] bg-[#080c08] flex items-center gap-2 justify-between shrink-0">
                  <div className="flex items-center gap-1.5">
                    {!review && (
                      <button onClick={() => patch(q.id, { phase: "coding", reviewMode: null })}
                        className="text-xs text-[#2a402a] hover:text-[#4d7a4d] tracking-wider transition-colors mr-2">
                        ← EDIT
                      </button>
                    )}
                    {review && qs.reviewMode !== null && (
                      <button onClick={() => patch(q.id, { reviewMode: null })}
                        className="text-xs text-[#2a402a] hover:text-[#4d7a4d] tracking-wider transition-colors mr-2">
                        ← BACK
                      </button>
                    )}
                  ...
                  {review ? (
                    ...
                    <div className="flex gap-2">
                      {RATINGS.map(({ value, label, cls }) => (
                        <button key={value} onClick={() => handleRate(value)} disabled={submitting !== null}
                          className={`px-3 py-1.5 text-xs border tracking-wider disabled:opacity-30 transition-colors ${cls}`}>
                          {submitting === value ? "···" : label}
                        </button>
                      ))}
