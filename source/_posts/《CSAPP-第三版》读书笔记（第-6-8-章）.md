...
* ***写回***：假设要写一个已经缓存了的字 w，在高速缓存更新了它的 w 副本之后，如何更新 w 在高速缓存较低层次中的副本呢？“写回”，即**尽可能地推迟更新，只有当替换算法要驱逐这个更新过的块时，才把它写到紧接着的低一层中**；
* ***写分配***：若写不命中，“写分配”会**加载相应的低一层中的块到当前层级的高速缓存中，然后再更新这个高速缓存块**（与写回相对应）。
...
* ***.text***：已编译程序的机器代码；
* ***.rodata***：只读数据；
* ***.data***：**已初始化的全局和静态 C 变量**。局部变量在运行时被保存在栈中；
...
* lib.c 中对变量 x 的赋值**将用负零的双精度浮点表示覆盖内存中 x 和 y 位置上的值**（x、y 的大小在 main.c 中为 4 个字节，而在 lib.c 中的浮点操作将为 x 替换连续的 8 个字节，因此会覆盖掉 y）；
* 
...
* 静态库解析的外部**引用规则**（即：左边文件引用右边文件中定义的符号）：在符号解析阶段，链接器**从左到右**扫描命令行中出现的可重定位目标文件（.c 文件会被自动翻译为 .o）与静态库文件，扫描过程中，链接器会维护一个**集合 E（所有输入文件）**、**集合 U（引用但未定义的符号）**、**集合 D（在前面输入文件中已定义的符号集合）**：
...
* 加载器从 **_start** 函数（ctrl.o）开始执行代码，然后调用系统启动函数 **__libc_start_main**（libc.so），然后执行 main 函数；
...
* ***终止***：进程永远地停止了。原因可能为：(1) 收到一个信号，其默认行为是终止进程；(2) 从主程序返回；(3) 调用 `exit` 函数。
