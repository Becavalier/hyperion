...
| ***div divisor*** | 无符号除法。 | *div word [0x0230]* | rdx:rax / divisor -> rax := 结果，rdx := 余数。 |
| ***mul [r/m]*** | 无符号乘法。 | *mov rax, 0x8<br>mul rax* | 结果分别存放到 rdx 与 rax。 |
| ***imul [r/m]*** | 有符号乘法。 | *mov rax, 0x8<br>imul rax* | 结果分别存放到 rdx 与 rax。 |
| ***neg src*** | 取负。 | *neg rax* | A neg instruction is the result of a *not* and *add* 1。 |
...
| ***cbw / cwde / cdqe*** | 符号扩展 al~eax 寄存器，并直接更新值。 | *cbw* | |
| ***cwd / cdq / cqo*** | 符号扩展 ax\~rax 寄存器，并将扩展的高位结果存放到 dx\~rdx 寄存器。 | *cwd* | |
...
| ***shl / sal*** | 左移。 | *shl eax, 2* | 移动时 LSB 补零。 |
| ***shr*** | 右移。 | *shr eax, 2* | 移动时 MSB 补零。 |
...
| ***movs / movsb / movsw / movsd / movsq*** | 移动一段线性内存数据。 | *rep movsb* | DF 标志位决定拷贝方向。 |
