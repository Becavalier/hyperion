...
class Quick {
  ...
  static int partition(vector<int>& v, int lo, int hi) {
    ...
    auto pivot = v[lo];
    int i = lo + 1, j = hi;
    // i，j 对向而行，若 i 找到一个大于 pivot 的值，且 j 找到一个小于 pivot 的值，则将两者交换；
    while (i <= j) {
...
class LFUCache {
  ...
  unordered_map<int, list<int>> freqToKeys;  // 存储的 key 需要有序，且支持快速删除。这里可以用一个 keyToListIter 的字典进行优化；
  int cap;
  void increaseFreq(int key) {
    auto freq = keyToFreq[key];  // 取出现在的 freq；
    ++keyToFreq[key];
    auto& keyList = freqToKeys[freq];
    keyList.remove(key);  // 先清除；
