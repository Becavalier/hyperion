...
class QuickSort {
  ...
  static int partition(vector<int>& v, int lo, int hi) {
    ...
    while (i <= j) {
      ...
      if (i < j) swap(v[i], v[j]);  // 则将两者交换，pivot 左侧均 <= 它；
    }
...
```cpp
// 快速选择会原地修改范围内的元素顺序，可能会影响题目后续计算过程；
class QuickSelect {
  static int quickSelect(
    ...
    const auto idx = p - left;
    if (idx == targetIdx) return *p;
    if (idx > targetIdx) return quickSelect(left, p - 1, targetIdx);
    // 这里 targetIdx 要减去 p 以及其左边部分；
    else return quickSelect(p + 1, right, targetIdx - idx - 1);
  }
  static vector<int>::iterator partition(vector<int>::iterator lo, vector<int>::iterator hi) {
    int p = *lo;
    auto left = lo + 1, right = hi;
    while (left <= right) {
      while (left <= hi && *left <= p) ++left;
      while (right >= lo && *right > p) --right;
      if (left >= right) break;
...
public:
  static int search(vector<int>::iterator left, vector<int>::iterator right, int targetIdx) {
    return quickSelect(left, right, targetIdx);
  }
};
```
