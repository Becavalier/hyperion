...
* *Meta-data*: token type, literal value, location information, and etc.
...
* Symbol (符号): 
  * Terminal (终结符): a letter from the grammar’s alphabet, like a literal value.
  * Nonterminal (非终结符): a named reference to another rule in the grammar, they are replaced by groups of terminal symbols according to the production rules.
* In a parse AST tree, every single grammar production becomes a node in the tree. *An AST elides productions that aren’t needed by later phases*.
...
14. BNF grammer with precedence (lowest -> highest) and associativity:
