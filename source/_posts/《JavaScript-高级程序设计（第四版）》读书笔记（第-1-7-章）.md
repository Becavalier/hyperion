...
9. （Page：95）包含在 *\<script\>* 内的代码会被从上到下解释。**在其内部的代码被计算完成之前，页面的其余内容不会被加载，也不会被显式**。同样的，在解释外部 JavaScript 文件时，页面也会阻塞。
...
* 对于 `+` 加法操作符，前者在操作数中存在字符串时，会对另一个操作数调用 String() 方法（间接调用 toString。null 与 undefined 无该方法）来将其转换为字符串，然后进行拼接。对于 `-` 减法操作符，则会调用对象操作数的 valueOf 或对其他类型操作数调用 Number() 方法来将其转换为数字。
...
* 所有的生成器对象都有 `return` 方法，借助于它和 `throw` 方法可用于强制生成器进入关闭状态（两者均需要**主动调用**）。其中，**若生成器还没有开始执行，那么调用 throw 抛出的错误不会在生成器函数内部被捕获**，这相当于在函数外部抛出了错误。
