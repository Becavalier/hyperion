...
export default function middlewares(...fns) {
  return function (ctx) {
    const dispatch = async function (idx) {
      const fn = fns[idx];
      if (!fn) return;
...
function checkIfInstanceOf(obj, classFunction) {
  if (obj == null) return false;  // 同时处理 null 和 undefined；
  if (classFunction == null || typeof classFunction !== 'function') return false;

  let proto = Object.getPrototypeOf(Object(obj));  // Object() 包装基本类型；
  while (proto !== null) {
...
Function.prototype.myApply = function (thisArg, args) {
  ...
  if (typeof this !== "function") throw new Error("only function is callable.");
  if (args != null && typeof args[Symbol.iterator] !== "function")
    throw new TypeError("non-iterable object is invalid");
...
每当有更新发生时，Reconciler（协调器）会：

* 调用函数组件 \ class 组件的 render 方法，将返回的 JSX 转化为虚拟 DOM；
* 将虚拟 DOM 和上次更新时的虚拟 DOM 对比（自顶向下，深度优先）；
* 通过对比找出本次更新中变化的虚拟 DOM；
* 通知 Renderer（渲染器）将变化的虚拟 DOM 渲染到页面上。

由于这个阶段中所有操作都是不可中断的同步操作，因此，当大组件树或大量更新时会导致页面卡顿或阻塞用户交互。


### React v16 的优化

架构中新增了 Scheduler（调度器），它决定任务的调度优先级。Reconciler 内部采用 Fiber 架构，可以将变化的虚拟 DOM 打上代表增/删/更新的标记。当所有组件都完成 Reconciler 的工作后，会被统一交给 Renderer 执行 DOM 更新操作。Scheduler 和 Reconciler 之间的工作全部在内存中进行，中间因为任务优先级，或者当前帧没有剩余时间可能会反复中断。Fiber 是 React 内部实现的一套状态更新机制。支持任务不同优先级，可中断与恢复，并且恢复后可以复用之前的中间状态。

### React Fiber

#### 双缓存

* React 使用双缓存技术完成 Fiber 树的构建与替换 —— 对应 DOM 树的创建与更新；

### JSX

JSX -> React.createElement -> React Element。JSX 描述了组件内容，在组件 mount 时，Reconciler 根据 JSX 描述的组件内容生成对应的 Fiber 节点。update时，Reconciler 将 JSX 与 Fiber 节点保存的数据对比，生成组件对应的 Fiber 节点，并根据对比结果为 Fiber 节点打上标记。
...
* 文章 [Node.js Internal](/2023/06/05/Nodejs-Internal/)；
...
| 3  | 上一帧中注册的所有 requestAnimationFrame 回调函数执行 | 在浏览器准备开始绘制前执行 |
...
* ***Requirements exploration***：15%。理解问题（把面试官当产品经理）；
  * *功能性需求*：完成产品功能的基本需求，分主要功能和附加功能；
  * *非功能性需求*：性能、可扩展性、用户体验等。
* ***Architecture / High-level design***：20%。识别关键组件和彼此之间的关系；
* ***Data model / Core entities***：10%。描述核心实体及其数据，每个实体包含的字段以及它们所属的组件；
  * 需要与服务端交互的持久化数据；
  * 表示前端应用状态的临时数据。
* ***Interface definition (API)***：15%。定义需要的 API - 功能、参数、响应；
* ***Optimizations and deep dive***：40%。讨论可能的优化点。
